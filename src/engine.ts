...
import { Action, CheckType, CriId, EngineResult, UserState } from './types';

const checkTypeOf = (criId: CriId): CheckType =>
  CRI_CONFIG.find(c => c.id === criId)!.checkType;
...
export const calculateNextStep = (state: UserState): EngineResult => {
  ...
  const succeededCheckTypes = state.succeeded.map(checkTypeOf);
  const hasCompletedMandatoryChecks = POLICY.MANDATORY_CHECKS.every(checkType =>
    succeededCheckTypes.includes(checkType)
  );

  const hasRequiredData = POLICY.REQUIRED_IDENTITY_DATA.every(d => state.hasData[d]);

  const hasCi = state.cis.length > 0;
  if (hasCi) {
    return {
      action: Action.STOP,
      message: `Sorry, you are blocked by CIs: ${state.cis.join(', ')}.`
    };
  }
  ...
  const eligibleChecks = CRI_CONFIG.filter(cri =>
    cri.enabled &&
    !state.attempted.includes(cri.id) &&
    cri.requires.every(dataType => state.hasData[dataType])
  );

  // Prioritise mandatory checks
  const mandatoryChecks = eligibleChecks.filter(cri =>
    POLICY.MANDATORY_CHECKS.includes(cri.checkType)
  );
  ...
  const scoreContributors = eligibleChecks.filter(cri =>
    contributesToScores(POLICY.TARGET_PROFILES, cri, state.scores)
  );
  ...
  const dataCollectors = eligibleChecks.filter(cri =>
    cri.collects.some(dataType => !state.hasData[dataType])
  );
